/**
 * @file src/interfaces/taskingai.js
 * @class TaskingAI
 * @description Wrapper class for the Tasking AI API.
 * @param {string} apiKey - The API key for Tasking AI.
 */
...
const { taskingAIApiKey } = require('../config/config.js');
const config = require('../config/llmProviders.json');
const log = require('loglevel');

// TaskingAI class for interacting with the Tasking AI API
class TaskingAI {
  /**
   * Constructor for the TaskingAI class.
   * @param {string} apiKey - The API key for the Tasking AI API.
   */
  constructor(apiKey) {
    this.interfaceName = 'taskingai';
    this.apiKey = apiKey || taskingAIApiKey;
    this.client = axios.create({
      baseURL: 'https://api.tasking.ai/v1/inference/chat_completion', // Tasking AI API base URL
      headers: {
  ...
  /**
   * Send a message to the Tasking AI API.
   * @param {string|object} message - The message to send or a message object.
   * @param {object} options - Additional options for the API request.
   * @param {object} interfaceOptions - Options specific to the interface.
   * @returns {string} The response content from the Tasking AI API.
   */
  async sendMessage(message, options = {}, interfaceOptions = {}) {
    ...
    while (retryAttempts >= 0) {
      try {
        // Send the request to the Tasking AI API
...
TaskingAI.prototype.adjustModelAlias = adjustModelAlias;

module.exports = TaskingAI;
